...
def train(dim_word=100, # word vector dimensionality
    ...
    if use_preemb:
        ...
        with open(embeddings, 'rb') as f:
            wv = pkl.load(f)
        wv_embs = numpy.zeros((len(list(worddict.keys())), len(list(wv.values())[0])), dtype='float32')
    ...
    if isinstance( dataset, str ):
        ...
        # provide loading function with returning three iterators
        assert callable(dataset), "Dataset is not a callable function"
        train, valid, test = dataset(batch_size=batch_size)
    ...
    if param_noise > 0.:
        ...
        add_update = []
        rem_update = []
        for vv, nn in zip(list(tparams.values()), list(noise_tparams.values())):
